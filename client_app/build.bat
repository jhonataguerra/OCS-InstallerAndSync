...
rem Detecta a versao do Windows para selecionar o compilador adequado.
...
if not exist "!CSC_EXE!" (
    echo [AVISO] Compilador nativo nao encontrado. Buscando alternativas...
    if exist "%CSC_4X%" (
        set "CSC_EXE=%CSC_4X%"
...
if exist "%CSC_35%" (
    ...
    "%CSC_35%" /target:winexe /platform:anycpu /optimize+ /win32manifest:"%~dp0app.manifest.xml" /out:"%~dp0CadastroPatrimonio_Win7_net35.exe" %REFERENCES% %SOURCES%
    if !ERRORLEVEL! neq 0 exit /b 1
)

if exist "%CSC_4X%" (
    echo [INFO] Compilando CadastroPatrimonio_Win10_net46.exe...
    "%CSC_4X%" /target:winexe /platform:anycpu /optimize+ /win32manifest:"%~dp0app.manifest.xml" /out:"%~dp0CadastroPatrimonio_Win10_net46.exe" %REFERENCES% %SOURCES%
    if !ERRORLEVEL! neq 0 exit /b 1

    echo [INFO] Compilando CadastroPatrimonio_Win11_net48.exe...
    "%CSC_4X%" /target:winexe /platform:anycpu /optimize+ /win32manifest:"%~dp0app.manifest.xml" /out:"%~dp0CadastroPatrimonio_Win11_net48.exe" %REFERENCES% %SOURCES%
    if !ERRORLEVEL! neq 0 exit /b 1
)
